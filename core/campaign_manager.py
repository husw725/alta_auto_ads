...
class CampaignManager:
    """二级优化加固版：1-1-5 赛马 + 完整调优大脑 (v2.10.3)"""
    ...
    def create_campaign(self, drama_name, materials_list):
        try:
            ...
            for idx, mat in enumerate(materials_list):
                ...
                video_data = {'video_id': v_id, 'message': main_copy.get('primary_text', f"Enjoy {drama_name}!"), 'title': main_copy.get('headline', f"Watch {drama_name} V{idx+1}"), 'call_to_action': {'type': 'INSTALL_APP', 'value': {'link': self.official_store_url}}}
    ...
    def evaluate_optimization_rules(self, campaigns, insights, history=None):
        ...
        for camp in campaigns:
            ...
            # 🚀 规则 2：降低预算 (CPI > T*0.8 且 Spend > $30)
            elif cpi > (CPI_T * 0.8) and spend > 30:
                new_b = curr_budget * 0.5
                actions.append({'type': 'BUDGET', 'cid': cid, 'name': camp['name'], 'value': new_b, 'reason': "CPI 预警，预算降 50%", 'risk': False})
            
            # 🚀 规则 3：提升预算 (CPI < T*0.6 且 ROI > Target*1.2)
            elif spend > MIN_S and cpi < (CPI_T * 0.6) and roi > (ROI_T * 1.2):
                new_b = curr_budget * 1.3
                actions.append({'type': 'BUDGET', 'cid': cid, 'name': camp['name'], 'value': new_b, 'reason': f"表现优异(ROI:{roi:.2f})，预算提 30%", 'risk': (new_b - curr_budget > 100)})

            # 🚀 规则 4：趋势分析 (需历史数据)
            if history and cid in history:
                h = history[cid]
                if len(h) >= 3 and all(d['cpi'] > CPI_T for d in h[-3:]):
                    actions.append({'type': 'BID', 'cid': cid, 'name': camp['name'], 'action': 'LOWER', 'reason': "CPI 连续 3 天超标，建议降价"})
                if len(h) >= 2 and h[-2]['imps'] > 0 and (h[-2]['imps'] - h[-1]['imps']) / h[-2]['imps'] > 0.3:
                    actions.append({'type': 'BID', 'cid': cid, 'name': camp['name'], 'action': 'HIGHER', 'reason': "展示量骤降 > 30%，建议加价"})
        return actions
